...
class SSPAICrawler:
    ...
    def get_articles(self, pages=10, limit=20):
        ...
        for page in range(pages):
            ...
            try:
                response = self.session.get(ARTICLE_API["index"], params=params, timeout=30)
    ...
    def get_article_detail(self, article_id):
        """获取文章详情"""
        try:
            response = self.session.get(ARTICLE_API["detail"], params={"id": article_id}, timeout=30)
            response.raise_for_status()
    ...
    def get_comments(self, article_id):
        """获取文章评论"""
        try:
            response = self.session.get(COMMENT_API, params={"article_id": article_id}, timeout=30)
            response.raise_for_status()
