...
# 导入其他模块
from app.models import Article, BestComment, ArticleResponse, CommentResponse
from app.database import create_db_and_tables, get_session, DatabaseHandler
...
def get_article_comments(article_id: int, session: Session = Depends(get_session)):
    """获取指定文章的所有评论"""
    comments = session.exec(select(BestComment).where(BestComment.article_id == article_id)).all()
    return comments
